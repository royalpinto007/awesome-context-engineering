...
<!-- LIST:START -->
**39 entries**, auto-refreshed weekly. Star counts updated **2026-09-07**. Browse the filterable version at **[context-engineering.agentpostmortem.com](https://context-engineering.agentpostmortem.com)**.
...
- [context-mode](https://github.com/mksglu/context-mode) `* 20.6k`: MCP server that sandboxes tool output (up to 98% token reduction) and persists session memory in SQLite with BM25 retrieval on compaction.
...
- [LlamaIndex](https://github.com/run-llama/llama_index) `* 52.1k`: Leading data framework for ingesting, indexing, and retrieving context to augment LLM output.
...
- [Mem0](https://github.com/mem0ai/mem0) `* 64.8k`: Lightweight open-source memory layer adding persistent memory to any LLM app over vector and relational storage.
- [Graphiti (Zep)](https://github.com/getzep/graphiti) `* 30.7k`: Temporal knowledge-graph memory that tracks how facts change over time with provenance.
- [Cognee](https://github.com/topoteretes/cognee) `* 30.6k`: Open-source memory framework building knowledge graphs from data for agent long-term memory.
- [Letta (formerly MemGPT)](https://github.com/letta-ai/letta) `* 24.6k`: Stateful-agent platform with main/recall/archival memory tiers the model pages in and out via tool calls.
...
- [ccusage](https://github.com/ryoppippi/ccusage) `* 18.4k`: CLI that parses Claude Code's local JSONL logs to report token usage and cost by day, session, and project.
...
- [DSPy (Stanford NLP)](https://github.com/stanfordnlp/dspy) `* 37.8k`: Framework for programming (not prompting) LLMs, with typed signatures, modules, and automated prompt/context optimization.
...
- [MetaGPT](https://github.com/FoundationAgents/MetaGPT) `* 70.2k`: Multi-agent framework that isolates context across role-specialized agents.
...
- [DeepEval](https://github.com/confident-ai/deepeval) `* 18.1k`: Pytest-style LLM eval framework with debuggable RAG and context metrics for CI/CD.
...
- [Context Rot toolkit (Chroma)](https://github.com/chroma-core/context-rot) `* 303`: Toolkit reproducing Chroma's finding that LLM reliability degrades non-uniformly as input tokens grow, across 18 models.
