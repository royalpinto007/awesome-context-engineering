...
- [context-mode](https://github.com/mksglu/context-mode) `* 23.8k`: MCP server that sandboxes tool output (up to 98% token reduction) and persists session memory in SQLite with BM25 retrieval on compaction.
- [hermes-lcm](https://github.com/stephenschoettler/hermes-lcm) `* 1.2k`: DAG-based context engine that compacts old context into depth-aware summary nodes without dropping messages.
...
- [LlamaIndex](https://github.com/run-llama/llama_index) `* 52.3k`: Leading data framework for ingesting, indexing, and retrieving context to augment LLM output.
...
- [Mem0](https://github.com/mem0ai/mem0) `* 65.8k`: Lightweight open-source memory layer adding persistent memory to any LLM app over vector and relational storage.
...
- [tiktoken (OpenAI)](https://github.com/openai/tiktoken) `* 19.3k`: OpenAI's official BPE tokenizer for exact local token counting to stay inside context windows and forecast cost.
- [tiktoken-rs](https://github.com/zurawiki/tiktoken-rs) `* 407`: Rust tokenizer library for GPT/tiktoken token accounting.
...
- [ccusage](https://github.com/ryoppippi/ccusage) `* 18.7k`: CLI that parses Claude Code's local JSONL logs to report token usage and cost by day, session, and project.
...
- [DSPy (Stanford NLP)](https://github.com/stanfordnlp/dspy) `* 38.2k`: Framework for programming (not prompting) LLMs, with typed signatures, modules, and automated prompt/context optimization.
- [FastMCP](https://github.com/jlowin/fastmcp) `* 27.8k`: Ergonomic Python framework for building MCP servers that feed context to agents.
- [MCP Python SDK](https://github.com/modelcontextprotocol/python-sdk) `* 24.4k`: Official MCP SDK for exposing tools and resources that agents select into context.

### Multi-agent and isolation

- [MetaGPT](https://github.com/FoundationAgents/MetaGPT) `* 70.5k`: Multi-agent framework that isolates context across role-specialized agents.
...
- [DeepEval](https://github.com/confident-ai/deepeval) `* 18.4k`: Pytest-style LLM eval framework with debuggable RAG and context metrics for CI/CD.
- [Ragas](https://github.com/explodinggradients/ragas) `* 15.8k`: Reference-free RAG evaluation with metrics for context relevance and precision, plus synthetic test-set generation.
...
- [Context Rot toolkit (Chroma)](https://github.com/chroma-core/context-rot) `* 308`: Toolkit reproducing Chroma's finding that LLM reliability degrades non-uniformly as input tokens grow, across 18 models.
